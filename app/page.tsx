...
export default function Home() {
  ...
  const [tradeSeed, setTradeSeed] = useState<{ symbol: string; assetClass: "equity" | "crypto" } | null>(null);
  ...
  return (
    <main className="app-shell">
      ...
      {tradeOpen && dashboard && <TradeDrawer key={`${tradeSeed?.assetClass || "equity"}:${tradeSeed?.symbol || "default"}`} portfolio={selectedPortfolio || dashboard.portfolio} dashboard={dashboard} initialSymbol={tradeSeed?.symbol} initialAssetClass={tradeSeed?.assetClass} onClose={() => setTradeOpen(false)} onComplete={async (message) => { setNotice(message); setTradeOpen(false); setActive("Overview"); await refreshDashboard(); }} onError={setError} />}
    </main>
  );
}
...
function BenchmarkManager({ dashboard, onRefresh, onNotice, onError }: { dashboard: Dashboard; onRefresh: () => Promise<void>; onNotice: (message: string) => void; onError: (message: string) => void }) {
  const suggestions = ["SPY", "QQQ", "DIA", "IWM", "VTI", "AAPL", "NVDA", "BTC-USD"], selected = dashboard.benchmarks.map((item) => item.symbol);
  ...
  function addBenchmark(raw: string) {
    const symbol = raw.trim().toUpperCase();
    if (!symbol || draft.includes(symbol)) return setSearch("");
    if (draft.length >= 8) return onError("You can compare up to eight benchmarks.");
  ...
  return <section className="panel benchmark-panel"><div className="panel-head"><div><span className="panel-title">Benchmark comparison</span><p>Search any supported ticker and compare up to eight symbols</p></div><button className="text-button" onClick={() => { if (!editing) setDraft(selected); setEditing((value) => !value); }}>{editing ? "Close" : "Manage"}</button></div>{editing ? <div className="benchmark-search-editor"><div className="benchmark-search"><input list="benchmark-suggestions" value={search} onChange={(event) => setSearch(event.target.value.toUpperCase())} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addBenchmark(search); } }} placeholder="Search ticker, e.g. VTI or BTC-USD" aria-label="Benchmark ticker"/><datalist id="benchmark-suggestions">{suggestions.filter((symbol) => !draft.includes(symbol)).map((symbol) => <option key={symbol} value={symbol}/>)}</datalist><button className="secondary-button" disabled={!search.trim() || draft.length >= 8} onClick={() => addBenchmark(search)}>Add</button></div><div className="benchmark-chips">{draft.map((symbol) => <button key={symbol} onClick={() => setDraft((current) => current.filter((item) => item !== symbol))} title={`Remove ${symbol}`}><span>{symbol}</span> ×</button>)}{!draft.length && <small>No benchmarks selected yet.</small>}</div><div className="benchmark-editor-actions"><span>{draft.length}/8 selected · Enter any ticker supported by the quote router.</span><button className="trade-button" disabled={saving} onClick={save}>{saving ? "Validating…" : "Save benchmarks"}</button></div></div> : dashboard.benchmarks.length ? <div className="benchmark-list">{dashboard.benchmarks.map(({ symbol, quote }) => <div key={symbol}><strong>{symbol}</strong><span>{money(Number(quote.mark))}</span><small>{quote.quality} · {quote.provider}</small></div>)}</div> : <div className="panel-empty compact"><strong>No benchmarks selected</strong><span>Build a comparison set whenever you are ready.</span></div>}</section>;
}

function MarketsPanel({ portfolioId, onTrade, onError }: { portfolioId: string; onTrade: (symbol: string, assetClass: "equity" | "crypto") => void; onError: (message: string) => void }) {
  const [items, setItems] = useState<WatchlistItem[]>([]), [selected, setSelected] = useState(""), [search, setSearch] = useState(""), [assetClass, setAssetClass] = useState<"equity" | "crypto">("equity"), [loading, setLoading] = useState(true);
  const load = useCallback(async () => {
    const response = await fetch(`/api/watchlist?portfolioId=${encodeURIComponent(portfolioId)}`), payload = await response.json() as { items?: WatchlistItem[]; error?: string };
    if (!response.ok) throw new Error(payload.error || "Unable to refresh watchlist.");
    setItems(payload.items || []); setSelected((current) => current && payload.items?.some((item) => item.symbol === current) ? current : payload.items?.[0]?.symbol || ""); setLoading(false);
  }, [portfolioId]);
  useEffect(() => { const initial = window.setTimeout(() => load().catch((reason) => onError(reason.message)), 0), interval = window.setInterval(() => load().catch(() => undefined), 30_000); return () => { window.clearTimeout(initial); window.clearInterval(interval); }; }, [load, onError]);
  async function add() {
    const symbol = search.trim().toUpperCase(); if (!symbol) return;
    const response = await fetch("/api/watchlist", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ portfolioId, symbol, assetClass: symbol.endsWith("-USD") ? "crypto" : assetClass }) }), payload = await response.json() as { error?: string };
    if (!response.ok) return onError(payload.error || "Unable to add symbol."); setSearch(""); setSelected(symbol); await load();
  }
  async function remove(symbol: string) {
    ...
    if (!response.ok) return onError(payload.error || "Unable to remove symbol."); await load();
  }
  const active = items.find((item) => item.symbol === selected) || items[0], points = active?.history || [], values = points.map((point) => point.value), low = values.length ? Math.min(...values) : 0, high = values.length ? Math.max(...values) : 0, range = Math.max(0.01, high - low);
  return <div className="markets-workspace"><section className="panel watchlist-panel"><div className="panel-head"><div><span className="panel-title">Portfolio watchlist</span><p>Persistent symbols · quotes refresh every 30 seconds</p></div><span className="quality-badge">{items.length}/24</span></div><div className="watchlist-add"><input list="watchlist-symbols" value={search} onChange={(event) => setSearch(event.target.value.toUpperCase())} onKeyDown={(event) => { if (event.key === "Enter") add(); }} placeholder="Search or type ticker" aria-label="Watchlist ticker"/><datalist id="watchlist-symbols">{[...equityTickers, ...cryptoTickers].map((symbol) => <option key={symbol} value={symbol}/>)}</datalist><select value={assetClass} onChange={(event) => setAssetClass(event.target.value as "equity" | "crypto")}><option value="equity">Equity</option><option value="crypto">Crypto</option></select><button className="trade-button" onClick={add} disabled={!search.trim()}>Add</button></div>{loading ? <div className="panel-empty compact">Loading watchlist…</div> : <div className="watchlist-rows">{items.map((item) => <button key={item.symbol} className={active?.symbol === item.symbol ? "selected" : ""} onClick={() => setSelected(item.symbol)}><span><strong>{item.symbol}</strong><small>{item.name}</small></span><span className="number"><strong>{money(Number(item.quote.mark))}</strong><small className={item.change >= 0 ? "positive" : "negative"}>{signedMoney(item.change)} · {pct(item.changePercent)}</small></span></button>)}</div>}</section><section className="panel instrument-detail">{active ? <><div className="panel-head"><div><span className="panel-title">{active.symbol} · {active.name}</span><p>{active.assetClass} · {active.quote.provider}</p></div><div className="instrument-actions"><span className="quality-badge">{active.quote.quality}</span><button className="secondary-button" onClick={() => remove(active.symbol)}>Remove</button><button className="trade-button" onClick={() => onTrade(active.symbol, active.assetClass)}>Trade</button></div></div><div className="instrument-quote"><div><span>Last</span><strong>{money(Number(active.quote.mark))}</strong></div><div><span>Session change</span><strong className={active.change >= 0 ? "positive" : "negative"}>{signedMoney(active.change)} · {pct(active.changePercent)}</strong></div><div><span>Bid / Ask</span><strong>{money(Number(active.quote.bid))} / {money(Number(active.quote.ask))}</strong></div><div><span>Session range</span><strong>{money(low)} – {money(high)}</strong></div></div><div className="price-chart" aria-label={`${active.symbol} current-session price chart`}>{points.length ? points.slice(-120).map((point, index, chartPoints) => <i key={`${point.at}:${index}`} title={`${new Date(point.at).toLocaleTimeString()} · ${money(point.value)}`} style={{ left: `${chartPoints.length === 1 ? 50 : index / (chartPoints.length - 1) * 100}%`, top: `${8 + (high - point.value) / range * 84}%` }}/>) : <span>Collecting the first quote sample…</span>}</div><div className="chart-axis"><span>Session open</span><span>{points.length} samples</span><span>Now</span></div></> : <div className="panel-empty"><strong>No symbols selected</strong><span>Add a ticker to start monitoring the current session.</span></div>}</section></div>;
}
...
function TradeDrawer({ portfolio, dashboard, initialSymbol, initialAssetClass, onClose, onComplete, onError }: { portfolio: Portfolio; dashboard: Dashboard; initialSymbol?: string; initialAssetClass?: "equity" | "crypto"; onClose: () => void; onComplete: (message: string) => void; onError: (message: string) => void }) {
  const [assetClass, setAssetClass] = useState(initialAssetClass === "crypto" ? "Crypto" : "Equity"), [symbol, setSymbol] = useState(initialSymbol || "NVDA"), [side, setSide] = useState<"buy" | "sell">("buy"), [orderType, setOrderType] = useState<"market" | "limit" | "stop" | "stop_limit">("market"), [quantity, setQuantity] = useState(initialAssetClass === "crypto" ? 0.1 : 100), [limitPrice, setLimitPrice] = useState(""), [stopPrice, setStopPrice] = useState(""), [quote, setQuote] = useState<Quote | null>(null), [submitting, setSubmitting] = useState(false);
