...
export async function GET(request: Request) {
  try {
    ...
    if (!portfolioId || !["transactions", "positions", "pnl", "closed-trades"].includes(report || "")) throw new Error("portfolioId and a valid report are required.");
    const exported = await exportPortfolioCsv(portfolioId, report as "transactions" | "positions" | "pnl" | "closed-trades");
