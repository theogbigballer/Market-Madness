...
export async function POST(request: Request) {
  try {
    const body = await request.json() as { portfolioId?: string; symbol?: string; assetClass?: AssetClass; side?: "buy" | "sell"; orderType?: "market" | "limit"; quantity?: number; limitPrice?: number; timeInForce?: "day" | "gtc"; optionContract?: OptionContract };
    ...
    const result = await placeOrder({ portfolioId: body.portfolioId, symbol: body.symbol, assetClass: body.assetClass, side: body.side, orderType: body.orderType, quantity: Number(body.quantity), limitPrice: body.limitPrice, timeInForce: body.timeInForce, optionContract: body.optionContract });
